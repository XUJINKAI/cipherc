...
namespace ciphercTest
{
    public class Hash_SM3_Test : TestBase
    {
        ...
        public void Hash_Text()
        {
            ...
            var expect = "66C7F0F462EEEDD9D1F2D46BDC10E4E24167C4875CF2F7A2297DA02B8F4BA8E0".GetBytes();
            Assert.Equal(expect, result.GetByteResult());
        }
        ...
        {
            var result = RunCommand($"hash --algr {hash} --inform hex --outform hex 0000");
            var expect = output_hex.GetBytes();
            Assert.Equal(expect, result.GetByteResult());
        }
